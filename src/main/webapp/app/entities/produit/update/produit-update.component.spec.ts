...
describe('Produit Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Magasin query and add missing value', () => {
      const produit: IProduit = { id: 456 };
      const magasin: IMagasin = { id: 15689 };
      produit.magasin = magasin;

      const magasinCollection: IMagasin[] = [{ id: 15923 }];
      jest.spyOn(magasinService, 'query').mockReturnValue(of(new HttpResponse({ body: magasinCollection })));
      const additionalMagasins = [magasin];
      const expectedCollection: IMagasin[] = [...additionalMagasins, ...magasinCollection];
      jest.spyOn(magasinService, 'addMagasinToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ produit });
      comp.ngOnInit();

      expect(magasinService.query).toHaveBeenCalled();
      expect(magasinService.addMagasinToCollectionIfMissing).toHaveBeenCalledWith(
        magasinCollection,
        ...additionalMagasins.map(expect.objectContaining)
      );
      expect(comp.magasinsSharedCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const produit: IProduit = { id: 456 };
      const magasin: IMagasin = { id: 8918 };
      produit.magasin = magasin;

      activatedRoute.data = of({ produit });
      comp.ngOnInit();

      expect(comp.magasinsSharedCollection).toContain(magasin);
  ...
  describe('Compare relationships', () => {
    describe('compareMagasin', () => {
      it('Should forward to magasinService', () => {
        ...
        jest.spyOn(magasinService, 'compareMagasin');
        comp.compareMagasin(entity, entity2);
        expect(magasinService.compareMagasin).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
