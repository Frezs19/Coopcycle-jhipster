...
describe('Cooperative e2e test', () => {
  ...
  const cooperativeSample = { nom: 'ADP quantifying', zone: 'Congolese Rhône-Alpes cross-platform' };
  ...
  describe('new Cooperative page', () => {
    ...
    it('should create an instance of Cooperative', () => {
      cy.get(`[data-cy="nom"]`).type('payment Plastic Grass-roots').should('have.value', 'payment Plastic Grass-roots');

      cy.get(`[data-cy="zone"]`).type('interface').should('have.value', 'interface');
