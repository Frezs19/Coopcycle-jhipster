...
describe('Produit e2e test', () => {
  ...
  const produitSample = { nom: 'Persevering compelling', description: 'PracticalX', prix: 52996 };
  ...
  describe('new Produit page', () => {
    ...
    it('should create an instance of Produit', () => {
      cy.get(`[data-cy="nom"]`).type('b Proactive').should('have.value', 'b Proactive');

      cy.get(`[data-cy="description"]`).type('disintermediate enable').should('have.value', 'disintermediate enable');

      cy.get(`[data-cy="prix"]`).type('41315').should('have.value', '41315');
